...
-- 阶段一：单表查询强化（10题）
-- 1. 指定列与别名：查询所有用户，只显示用户名和注册日期，列别名分别为“姓名”、“注册时间”。提示：select 列名 as 别名。

select username as '姓名', reg_date as '注册时间' from users;



-- 2. 条件 >：找出所有账户余额大于 3000 的用户。提示：where balance > 3000。

select * from users where balance > 3000;



-- 3. or 与 in：查询属于“手机数码”或“图书音像”分类的所有商品（用 or 和 in 各写一版）。提示：where category = '手机数码' or category = '图书音像'；where category in ('手机数码','图书音像')。
...
-- 4. 模糊查询 like：找出商品名称中包含“电”字的商品。提示：where name like '%电%'。

select * from products where prod_name like '%电%';


-- 5. between and + 排序：查询单价在 100 到 1000 元之间的所有商品，按价格降序排列。提示：where price between 100 and 1000 order by price desc。

select * from products where price between 100 and 1000 order by price desc;



-- 6. 分组统计 + having：按会员等级统计人数，只显示人数,等级。
...
-- 7. 分页 limit：查询商品表，每页显示 3 条记录，写出获取第 2 页数据的 sql。提示：limit 3 offset 3 或 limit 3,3。

select * from products limit 2,3;



-- 8. 字符串函数：将每个用户的用户名转为大写，并拼接上“_VIP”后缀作为新列显示。提示：concat(upper(username), '_VIP')。

select concat(upper(username), '_VIP') from users;



-- 9. 日期函数 + 条件：计算每个订单的下单日期距离今天过了多少天，并只显示最近 7 天内的订单。提示：datediff(curdate(), order_date)，并用 where 过滤差值 <= 7。
...
-- 10. 流程函数：查询用户表，根据余额显示评价：余额 > 5000 显示“高净值”，否则显示“普通”（用 if 或 case when 实现）。提示：if(balance>5000,'高净值','普通')；或 case when balance>5000 then '高净值' else '普通' end。
...
-- 阶段二：多表连接（5题）
-- 1. 内连接：查询所有订单的订单编号、下单时间、下单用户姓名（显式 join 写法）。提示：orders inner join users on orders.user_id = users.id。
...
-- 2. 左连接 + 分组：查询所有用户及其历史总消费金额（没有下过单的用户总消费显示 0）。提示：left join 后按用户分组，对金额 sum，用 coalesce(sum(amount),0) 处理 null。
...
-- 3. 自连接：查询每个商品分类的名字，同时显示出它的上级分类名称（categories 表通过 parent_id 自关联）。提示：from categories c left join categories p on c.parent_id = p.id。
...
-- 4. 联合查询：将“手机数码”分类下的商品和“图书音像”分类下的商品，合并成一张列表（商品名、价格），用 union all 实现。提示：两个 select 语句用 union all 连接，列数和类型需一致。
...
-- 5. 连接库存表：查询每个商品的名称、价格以及对应的库存数量。提示：products join inventory on products.id = inventory.product_id，选择合适的连接类型。
...
-- 阶段三：子查询专项突破（15题）
-- 3-1 标量子查询
-- 1. 查询价格高于所有商品平均价格的商品名称和价格。提示：where price > (select avg(price) from products)。
...
-- 2. 找出账户余额最多的用户姓名和余额（必须用标量子查询，不能直接排序取第一条）。提示：where balance = (select max(balance) from users)。
...
-- 3. 查询下过订单的所有用户姓名。提示：where id in (select distinct user_id from orders)。
...
-- 4. 查询从未下过订单的用户，分别用 not in 和 not exists 写出两个版本。提示：not in 版本需注意子查询不要返回 null；not exists 使用关联子查询。
...
-- 5. （进阶验证）执行 update orders set user_id = null where order_id = 5; 后，重新运行第4题的两个版本，解释结果为何不同。提示：当子查询结果含 null 时，not in 会返回空集；not exists 不受影响。
...
-- 6. 查询价格比“家用电器”分类下所有商品都贵的商品。提示：where price > all (select price from products where category='家用电器')。
...
-- 7. 查询价格比“手机数码”分类下任意一个商品便宜的商品。提示：where price < any (select price from products where category='手机数码')。
...
-- 3-3 行子查询
-- 8. 查询与“张三”在同一天注册，且余额也相同的其他用户（排除张三自己）。提示：where (reg_date, balance) = (select reg_date, balance from users where name='张三') and name <> '张三'。
...
-- 3-4 表子查询（派生表）
-- 9. 先构建“每个用户的总消费金额”派生表，再查询消费总额高于所有用户平均消费总额的用户。提示：子查询先 group by user_id 求总消费，外层 where total > (select avg(total) from 派生表)。
...
-- 10. 在订单明细表中，将订单商品件数 >= 2 的订单做成派生表，关联订单主表，显示订单编号和总金额。提示：from orders join (select order_id from order_items group by order_id having sum(quantity) >= 2) as t on orders.id = t.order_id。
...
-- 3-5 关联子查询（核心能力）
-- 11. 查询每个商品分类中，价格高于该分类平均价格的商品（商品名、价格、分类名），必须用相关子查询。提示：where price > (select avg(price) from products p2 where p2.category_id = p1.category_id)。
...
-- 12. 用 exists 查询那些至少购买过一次单价超过 1000 元商品的用户。提示：where exists (select 1 from orders join order_items on ... where orders.user_id = users.id and price > 1000)。
...
-- 13. 查询与“张三”购买过同款商品的其他用户（需显示用户id、用户名及相同商品名，排除张三自己）。提示：用 exists 或 in 在订单明细中匹配张三购买过的 product_id，且用户不是张三。
...
-- 阶段四：事务（3个任务）
-- 1. 转账事务原子性：从“张三”账户转 500 元到“李四”账户。在事务中故意插入一句错误 sql，验证回滚后双方余额是否恢复原值。提示：start transaction; update ...; 错误语句; rollback; 检查值未变。
...
-- 2. 下单扣库存事务：模拟用户“王五”购买 2 个“电饭煲”：查询库存→判断充足→扣减库存→创建订单→插入明细。任一步失败则全部回滚。提示：整个流程包裹在事务中，任一失败触发 rollback，成功则 commit。
